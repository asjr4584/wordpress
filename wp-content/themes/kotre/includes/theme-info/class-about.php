...
if ( ! class_exists( 'Kotre_About' ) ) {
	...
	class Kotre_About {
		...
		/**
		 * Singleton instance of Kotre_About.
		 *
		 * @var Kotre_About $instance Kotre_About instance.
		 */
		private static $instance;

		/**
		 * Main Kotre_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Kotre_About ) ) {
				self::$instance = new Kotre_About;
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : get_template_directory_uri() . '/includes/theme-info/images/wpcharms.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://wpcapsules.com/vendor/wpcharms/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'kotre' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'kotre' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function render_about_page() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap wp-charms-wrap">';
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'kotre-about', get_template_directory_uri() . '/includes/theme-info/css/about.css', array(), '2.0.2' );
				wp_enqueue_script( 'kotre-about', get_template_directory_uri() . '/includes/theme-info/js/about.js', array( 'jquery' ), '2.0.2' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'kotre-about', 'KotreAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'kotre' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'kotre' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'kotre' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'kotre' );
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Free','kotre' ) . '</th>';
		            echo '<th>' . esc_html__( 'Pro','kotre' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="https://wpcapsules.com/item/kotre-pro-elementor-and-gutenberg-ready-multi-purpose-wordpress-theme/" target="_blank" class="button button-primary button-hero">Kotre Pro</a></td>';
		            echo '</tr>';
